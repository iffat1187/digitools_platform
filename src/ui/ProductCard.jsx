...
const ProductCart = ({ product, addToCart, setAddToCart }) => {
	const { name, description, price, period, tag, features, icon } =
		product || {}
		...
		<section>
			<div className={`card mx-auto w-full shadow-lg relative  rounded-xl `}>
				...
				<div className={`card-body space-y-1 `}>
					...
					<div className='w-12.5 h-12.5 flex flex-2  '>
						<img src={icon} alt={name} />
					</div>
					<div>
						<div className='space-y-3 '>
							<p className='text-xl md:min-h-15 lg:min-h-0 font-bold mt-2'>
								{name}
							</p>

							<p className={`text-gray-500 md:min-h-20  lg:min-h-15`}>
								{description}
							</p>
						</div>
					</div>
					...
					<div className='mt-3'>
						<button
							...
							className={`w-full rounded-full px-5 py-2 cursor-pointer font-bold  ${isSelected ? 'bg-green-500 text-white' : 'bg-linear-to-r from-[#4F39F6] to-[#9B5CF6] text-white'}
              `}
						>
							{isSelected ? (
